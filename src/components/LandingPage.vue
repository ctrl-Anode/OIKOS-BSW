  ...
  <div class="min-h-screen bg-gradient-to-b from-purple-400 via-pink-300 to-yellow-200">
    ...
    <header class="bg-purple-700 text-white py-3 px-4 sm:py-4 sm:px-6 shadow-md">
      <div class="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0">
        <h1 class="text-xl sm:text-2xl font-bold text-center sm:text-left">Sight Word Buddy</h1>
        <nav>
    ...
    <main class="px-4 sm:px-6 py-10 sm:py-12 max-w-6xl mx-auto">
      ...
      <!-- Accessibility -->
      <section id="accessibility" class="bg-gradient-to-r from-blue-300 to-green-300 rounded-3xl p-6 sm:p-12 shadow-2xl mb-16">
        <h2 class="text-3xl sm:text-4xl font-black text-center text-blue-900 mb-6 sm:mb-8">
          Built for Everyone! 🌈
        </h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div v-for="(access, index) in accessibility" :key="index" class="flex items-start gap-3 sm:gap-4">
            <div class="text-2xl sm:text-3xl">{{ access.icon }}</div>
            <div>
              <h3 class="text-lg sm:text-xl font-black text-blue-900 mb-1">{{ access.title }}</h3>
              <p class="text-base sm:text-lg text-blue-800">{{ access.description }}</p>
            </div>
          </div>
        </div>
      </section>
...
const features = ref([
  { icon: '🔤', title: 'Phonics Fun', description: 'Click on letters to hear their sounds and learn phonics interactively!', animationClass: 'animate-wiggle' },
  { icon: '🔊', title: 'Word Sounds', description: 'Tap on words to hear them spoken aloud with clear pronunciation.', animationClass: 'animate-pulse' },
  { icon: '👆', title: 'Swipe to Learn', description: 'Swipe words for pronunciation guides with slowed or segmented audio.', animationClass: 'animate-bounce' },
  { icon: '📊', title: 'Track Progress', description: "See how many words you've mastered and keep improving every day!", animationClass: 'animate-wiggle' },
  { icon: '🎮', title: 'Easy Controls', description: 'Play, pause, and replay audio anytime. Simple controls for little hands!', animationClass: 'animate-pulse' },
  { icon: '📱', title: 'Works Everywhere', description: 'Use on phones, tablets, or computers. Touch or click - both work great!', animationClass: 'animate-bounce' }
])
...
const accessibility = ref([
  { icon: '♿', title: 'Screen Reader Support', description: 'Works perfectly with assistive technologies' },
  { icon: '🎯', title: 'WCAG Compliant', description: 'Meets accessibility standards for all learners' },
  { icon: '🎨', title: 'High Contrast', description: 'Easy to see colors and clear text for everyone' },
  { icon: '🗣️', title: 'Text-to-Speech', description: 'Backup audio system works on all devices' }
])
